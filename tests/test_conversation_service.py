...
class TestHandleInboundMessage:
    def test_uses_existing_open_conversation(self):
        ...
        svc.handle_inbound_message(db=db, message=message, contact_id=CONTACT_ID, event_id=1)
    ...
    def test_creates_new_conversation_when_none_open(self):
        ...
        svc.handle_inbound_message(db=db, message=message, contact_id=CONTACT_ID, event_id=2)
    ...
    def test_last_message_at_is_updated(self):
        ...
        svc.handle_inbound_message(db=db, message=_make_message(), contact_id=CONTACT_ID, event_id=3)
    ...
    def test_raw_text_extracted_from_text_message(self):
        ...
        msg_obj: Message = next(c[0][0] for c in db.add.call_args_list if isinstance(c[0][0], Message))
    ...
    def test_new_conversation_has_correct_fields(self):
        ...
        svc.handle_inbound_message(db=db, message=_make_message(), contact_id=CONTACT_ID, event_id=7)

        conv_obj: Conversation = next(c[0][0] for c in db.add.call_args_list if isinstance(c[0][0], Conversation))
...
class TestExtractText:
    def test_text_message(self):
        assert ConversationService._extract_text({"type": "text", "text": {"body": "Hello"}}) == "Hello"

    def test_image_with_caption(self):
        assert ConversationService._extract_text({"type": "image", "image": {"caption": "foto"}}) == "foto"
