...
def _build_ai_engine(db: Session) -> AIEngine | None:
    ...
    if not key:
        return None
    try:
        llm = LLMClient(api_key=key, base_url=os.environ.get("OPENROUTER_BASE_URL","https://openrouter.ai/api/v1"), model=os.environ.get("LLM_MODEL","google/gemini-2.0-flash-001"))
...
def get_evolution_service(db: Session = Depends(get_db)) -> EvolutionWebhookService:
    return EvolutionWebhookService(db=db, evolution_api_key=EVOLUTION_API_KEY, account_id=ACCOUNT_ID, conversation_service=ConversationService(), ai_engine=_build_ai_engine(db))

@router.post("/evolution/whatsapp", status_code=status.HTTP_200_OK)
async def receive_evolution_webhook(request: Request, apikey: str|None = Header(None, alias="x-app-secret"), evolution_service: EvolutionWebhookService = Depends(get_evolution_service)) -> JSONResponse:
    try:
        payload = await request.json()
    except Exception:
        raise HTTPException(status_code=400, detail="Payload JSON inv�lido.")
    try:
