...
class ConsentRepository:
    ...
    def create(self, *, contact_id: uuid.UUID, conversation_id: uuid.UUID | None, type: str, status: str, purpose: str | None = None, legal_basis: str = "consent", channel_message_id: str | None = None) -> Consent:
    ...
    def get_latest(self, *, contact_id: uuid.UUID, type: str) -> Optional[Consent]:
        stmt = select(Consent).where(Consent.contact_id == contact_id, Consent.type == type).order_by(Consent.created_at.desc()).limit(1)
